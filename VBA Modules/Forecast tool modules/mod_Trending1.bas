Attribute VB_Name = "mod_Trending1"
Option Explicit
...
'
'        arr_Y_minus_Mean_of_LN_Y(intLoop - 1) = Application.WorksheetFunction.Ln(rngData.Cells(1, intLoop) - (Min_Data - 1))
'        arr_LN_Y_X(intLoop - 1) = Application.WorksheetFunction.Ln(1 / rngData.Cells(1, intLoop) - 1 / U)
'    Next intLoop
'
'    For intLoop = 1 To Data_Count
...
'            arr_ERR(intLoop - 1) = (arr_Calcs(intLoop - 1) - rngData.Cells(1, intLoop)) ^ 2
'        Else
'            arr_ERR(intLoop - 1) = 0
'        End If
'    Next intLoop
...
Function Trend_UserForm()
  frm_Penetration_Type.Show
End Function

Function Trend_Closed_UserForm()
...
Function Trend_Ratio_UserForm()
Dim shpCaller As String
    shpCaller = Application.Caller
    Application.ScreenUpdating = False
    Application.Calculation = xlCalculationManual
    
    If shpCaller = "shp_RR" Then
        wks_Trending.Range("C1").Value = "C12"
        wks_Trending.Range("A1").Value = 11
    ElseIf shpCaller = "shp_AR" Then
        wks_Trending.Range("C1").Value = wks_Trending.Range("B2").Value
        wks_Trending.Range("A1").Value = 379
    ElseIf shpCaller = "shp_VR" Then
        wks_Trending.Range("C1").Value = wks_Trending.Range("B3").Value
        wks_Trending.Range("A1").Value = 386
    End If
    ''frm_T2.Show
    Application.Calculation = xlCalculationAutomatic
    Application.ScreenUpdating = True
...
Function Navigate_Sheets()
    Dim strSheet As String
    strSheet = ActiveSheet.Shapes(Application.Caller).TextFrame.Characters.Text
    If strSheet = "Naive Government" Then
        Sheets("Control_2").Activate
    ElseIf strSheet = "Naive Commercial-PBM" Then
        Sheets("Control_1").Activate
    ElseIf strSheet = "Returning Government" Then
        Sheets("Control_4").Activate
    ElseIf strSheet = "Returning Commercial-PBM" Then
        Sheets("Control_3").Activate
    End If
End Function
