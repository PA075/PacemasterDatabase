Attribute VB_Name = "mod_Z_Transition"
Option Explicit
...
Dim vTimeInLag As Variant, vLaunch As Variant, vShr() As Variant, vSumShr() As Variant, vNormShr() As Variant, _
    vTime As Variant
Dim TotComp As Integer, TotTime As Integer, intI As Integer, j As Integer
...
ReDim vNormShr(1 To TotComp, 1 To TotTime)
    ...
    For j = 1 To TotTime
        ...
        For intI = 1 To TotComp
            ...
            dMax = rShr(intI, j)
            'dMax = rShr(intI, j).Value
            dTimeToMax = rTTP(1, j)
            dTimeInLag = rLag(1, j)
            vLaunch = rLaunch(intI, j)
            vTime = rTime(j)
            'vTime = rTime(j).Value
            
            vShr(intI, j) = MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, vTime, dInterval)
            'vShr(intI, j) = MB_Transition(dType, dStart(intI, j), dMax(intI, j), dTimeToMax, vTimeInLag, vLaunch, vTime, dInterval)
            vSumShr(j) = vSumShr(j) + vShr(intI, j)
    ...
    For j = 1 To TotTime
        ...
        For intI = 1 To TotComp
            ...
            If vSumShr(j) = 0 Then
                vNormShr(intI, j) = 0
            Else
                vNormShr(intI, j) = vShr(intI, j) / vSumShr(j)
            End If
...
ReDim vNormShr(1 To TotComp, 1 To TotTime)
    ...
    For j = 1 To TotTime
        ...
        For intI = 1 To TotComp
            ...
            dStart = rStart(intI)
            dMax(intI, j) = rShr(intI, j).Value
            vLaunch = rLaunch(intI)
            vTime = rTime(j).Value
            
            vShr(intI, j) = MB_Transition(dType, dStart, dMax(intI, j), dTimeToMax, vTimeInLag, vLaunch, vTime, dInterval)
            vSumShr(j) = vSumShr(j) + vShr(intI, j)
    ...
    For j = 1 To TotTime
        ...
        For intI = 1 To TotComp
            ...
            If sEvt = "BL" Then
                ...
                dType = Range("uptake_Comp" & intI & "_Seg" & SegNum & "_PG" & PGNum & "_G" _
                    & GeoNum).Value
                dStart = Range("dim_shrStart_Comp" & intI & "_Seg" & SegNum & "_PG" & PGNum & "_G" _
                    & GeoNum).Value
                dTimeToMax = Range("ttp_Comp" & intI & "_Seg" & SegNum & "_PG" & PGNum & "_G" _
                    & GeoNum).Value
                vTimeInLag = Range("lag_Comp" & intI & "_Seg" & SegNum & "_PG" & PGNum & "_G" _
                    & GeoNum).Value
                vLaunch = Int(rLaunch(intI).Value)
            ...
            Else
                ...
                Else
                    ...
                    If j = 1 Then
                        dStart = Range("dim_shrStart_Comp" & intI & "_Seg" & SegNum & "_G" & GeoNum).Value
                    Else
                        If rEvt(j) <> rEvt(j - 1) Then
                            dStart = vShr(intI, j - 1)
                        End If
...
On Error GoTo ErrFound
    ...
    If dTypeofForecast = "Yearly" Then
        ...
        If vLaunch <= DateAdd("M", -2, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -2, vTime), dInterval)
        If vLaunch <= DateAdd("M", -3, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -3, vTime), dInterval)
        If vLaunch <= DateAdd("M", -4, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -4, vTime), dInterval)
        If vLaunch <= DateAdd("M", -5, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -5, vTime), dInterval)
        If vLaunch <= DateAdd("M", -6, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -6, vTime), dInterval)
        If vLaunch <= DateAdd("M", -7, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -7, vTime), dInterval)
        If vLaunch <= DateAdd("M", -8, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -8, vTime), dInterval)
        If vLaunch <= DateAdd("M", -9, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -9, vTime), dInterval)
        If vLaunch <= DateAdd("M", -10, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -10, vTime), dInterval)
        If vLaunch <= DateAdd("M", -11, vTime) Then P_MB_Transition = P_MB_Transition + MB_Transition(dType, dStart, dMax, dTimeToMax, vTimeInLag, vLaunch, DateAdd("M", -11, vTime), dInterval)
