<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<PharmaACE.ForecastApp.Models.AutocompleteListData>" %>
...
<script>
    $(document).ready(function () {
        ...
        productNameList = JSON.parse('<%=Model.ProductList%>');
        pharmaClassNameList =JSON.parse('<%=Model.PharmaClassList%>');
        indicationNameList =JSON.parse('<%=Model.IndicationList%>');
        moleculeNameList = JSON.parse('<%=Model.MoleculeList%>');
        autocompleteListData =JSON.parse('<%=Model.ProductList%>');

        
        $("#search-filter-dropdown").change(function () {
            var Selectedfilter = this.value;
            Models.AutocompleteListData
            if (Selectedfilter == 1)
                autocompleteListData = JSON.parse('<%=Model.ProductList%>');
            else if (Selectedfilter == 2)
                autocompleteListData = JSON.parse('<%=Model.MoleculeList%>');
            else if (Selectedfilter == 3)
                autocompleteListData = JSON.parse('<%=Model.PharmaClassList%>');
            else if (Selectedfilter == 4)
                autocompleteListData =JSON.parse( '<%=Model.IndicationList%>');
        });
    });
</script>
