...
(function( $ ) {
    ...
    $.tagging = {
        $TAGGING_TAG: $('<li class="tagging_tag" title=""></li>')
        , TAG_DELETE: '<span class="tag_delete">x</span>'
        , $TAGGING_NEW: $(
            '<li class="tagging_new"><input type="text" class="tagging_new_input form-control" /></li>')
    ...
    function add_tag($input) {
      ...
      var inparent = $input.parents('li');
      var ul= inparent.parents('ul');
      var flag = 0;
      $(ul).find('li.tagging_tag').each(function()
      {
          if ($(this)[0].innerText.slice(0, -1) == value)
          {
              flag=1;
          }
      }
      ...
      if (flag == 0) {
          $new_tag.attr("title", value);
          $new_tag.html(value + $.tagging.TAG_DELETE);
          var maindiv = ul.parents('div');
          var divid = maindiv.get(1).id;
          if (divid == "indicationarray")
          {
              if ($.inArray(value, indicationNameList1) != -1)
              {
