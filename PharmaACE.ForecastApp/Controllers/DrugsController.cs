...
namespace PharmaACE.ForecastApp.Controllers
{
    public class DrugsController : BaseController
    {
     ...
     [OutputCache(Duration = 7200, Location = OutputCacheLocation.Server, VaryByParam = "searchModule;searchKey;searchParam;searchCondition;switchView;category")]
        ...
        public JsonResult AddInlineDurgs(ProductDetail dr)
        {
            ...
            try
            {
                ...
                if (result == 1)
                {
                    logger.Info("Successfully added inline drug");
                }
                else if (result == 2)
                {
                    logger.Info("Inline drug already added");
                }
                else
                {
                    msg = "fail to add inline drug";
                    logger.Info(msg);
                }
        ...
        public JsonResult EditInlineDurgs(ProductDetail dr)
        {
            logger.Info("Inside Drugs/EditInlineDurgs");
            string msg = String.Empty;
            int result = 0;
            try
            {
                result = new KnowledgeManager(UnitOfWork).EditInlineDurgs(dr);
                if (result == 1)
                {
                    logger.Info("Successfully added inline drug");
                }
