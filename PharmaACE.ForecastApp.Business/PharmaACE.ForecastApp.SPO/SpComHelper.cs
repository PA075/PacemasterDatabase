...
namespace PharmaACE.ForecastApp.Business
{
    public class SpComHelper
    {
        ...
        public static bool CreateDocumentLibrary(string spSiteUrl,
            string spSiteUsername,
            string spSitePwd,
            string spSiteDomain,
            bool isSpOnline, 
            string docLibName, 
            List<string> hierarchyManagers,
            out string msg)
        {
            msg = "";
            try
            {
                ...
                lci.Description = docLibName;
                lci.Title = docLibName;
                lci.TemplateType = 101;
                List newLib = ctx.Web.Lists.Add(lci);
                ctx.Load(newLib);
                var lstModelLocations = ForecastFactory.GetModelLocations();
                foreach (var location in lstModelLocations)
                {
                    //string folderUrl = GenUtil.ForecastModelServerRelativeUrl + docLibName + "/" + location + "/" + GenUtil.ForecastFolder + "/";
                    string rootUrl = GenUtil.ForecastModelServerRelativeUrl + docLibName + "/";
                    var rootFolder = ctx.Web.GetFolderByServerRelativeUrl(rootUrl);
                    var modelStorageFolder = rootFolder.Folders.Add(location);
                    var flatFileStorageFolder = modelStorageFolder.Folders.Add(GenUtil.ForecastFolder);
                    var attachmentStorageFolder = modelStorageFolder.Folders.Add(GenUtil.AssumptionAttachmentsFolder);
                }
                ctx.ExecuteQuery();
            }
            catch (Exception ex)
        ...
        //    string folderUrl,
        //    int sortCol,
        //    Dictionary<string, UserForecastMapping> userForecastMapping,
        ...
        //            {
        //                var forecastName = System.IO.Path.GetFileNameWithoutExtension(curFile.Name);
        //                if (!userForecastMapping.ContainsKey(forecastName))
        //                    continue;
        //                lstObjs.Add(new ForecastFlatFile()
        //                {
        ...
        //                    Author = userForecastMapping[forecastName].Creator.Email,
        ...
        //                    AuthorizationForLoggedInUser = userForecastMapping[forecastName].AuthenticationLevel
        //                });
        ...
        public static string GetLatestDocumentVersion(
            ClientContext ctx,
            string fileUrl,            
            out string msg)
        {
            if (ctx == null)
                throw new Exception("Could not connect to the forecast storage");
            ...
            try
            {
                ...
                ctx.Load(file, y => y.UIVersionLabel);
                ctx.ExecuteQuery();
                latest = String.Format("V{0}", file.UIVersionLabel);
            }
            catch (Exception ex)
            {
                msg = ex.Message;
        ...
        public static bool UploadFilesToSharePoint(
            ...
            List<System.IO.Stream> sourceFiles,
            ...
            out string msg)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < sourceFiles.Count; i++)
                {
                    ...
                    fci.ContentStream = sourceFiles[i];
                    fci.Url = destFileNames[i];
                    fci.Overwrite = true;
                    latestFile = folder.Files.Add(fci);
                    newSpFiles.Add(latestFile);
                    ctx.Load(latestFile, y => y.MajorVersion, y => y.MinorVersion);
                  
                    //latestVersion = "V" + (latestFile.MajorVersion + latestFile.MinorVersion);
                }
                ctx.RequestTimeout = GenUtil.SPTimeOut;
                ctx.ExecuteQuery(); //unlock
               // latestVersion = String.Format("{0}{1}{2}", "V", latestFile.MajorVersion, latestFile.MinorVersion);
                latestVersion = "V" + latestFile.MajorVersion + "." + latestFile.MinorVersion;
        ...
        public static bool GetOlderVersion(
            string spSiteUrl, 
            string spSiteUsername, 
            string spSitePwd,
            string fileServerRelUrl,
        ...
        public static ClientContext GetSPClientContext(string spSiteUrl, string spSiteUsername, string spSitePwd)
        {
            if (String.IsNullOrWhiteSpace(spSiteUrl) || String.IsNullOrWhiteSpace(spSiteUsername) || String.IsNullOrWhiteSpace(spSitePwd))
                return null;
            var ctx = new ClientContext(spSiteUrl);
